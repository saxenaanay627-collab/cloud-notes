...
Writing Data: Uses addDoc to push new notes to the cloud.

Reading Data: Uses a query ordered by timestamp so the newest notes always appear at the top.

Deleting Data: Uses deleteDoc with the specific document ID provided by Firebase.

 Setup Instructions
Clone or Copy: Save the index.html file to your local machine.

Firebase Config: Ensure the firebaseConfig object in the script tag matches your project credentials from the Firebase Console.

Database Rules: Set your Firestore rules to "Test Mode" or public for development:
